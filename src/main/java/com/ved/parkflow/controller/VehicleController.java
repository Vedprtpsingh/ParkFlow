...
model.addAttribute("availableSlots", parkingSlotService.getAllParkingSlots());
        return "parkVehicle";
    }
/***
...
**/
    ...
    @GetMapping("/addVehicleParkingSlot")
    public String showAddParkingSlotPage(Model model) {
        model.addAttribute("parkingSlot", new ParkingSlot());
        return "addParkingSlot";
    }

    @PostMapping("/addVehicleParkingSlot")
    public String addParkingSlot(@ModelAttribute("parkingSlot") ParkingSlot parkingSlot, Model model) {
        if (parkingSlot.getNumberOfSlots() <= 0) {
            model.addAttribute("error", "Number of slots must be greater than zero.");
            return "addParkingSlot";
        }
        if (parkingSlot.getLatitude() == 0.0 || parkingSlot.getLongitude() == 0.0) {
            model.addAttribute("error", "Latitude and Longitude are required.");
            return "addParkingSlot";
        }
        // Assuming ParkingSlot entity has a boolean field 'available' - if not, remove this line
        ...
        if (status) {
            ...
            logger.info("Parking slot added: numberOfSlots=" + parkingSlot.getNumberOfSlots());
        } else {
            model.addAttribute("error", "Failed to add parking slot.");
            logger.warning("Failed to add parking slot: numberOfSlots=" + parkingSlot.getNumberOfSlots());
        }
        return "addParkingSlot";
    }
